...
static int trace_ast_down_up_gen_3_address_code(Scope *scp, Ast *p)
{
	if (!p || p->sem == SEM_VAR_DECLARE)
		return -1;

	vector<ThreeAddrCode*> &tacs = scp->tacs;

	int b = trace_ast_down_up_gen_3_address_code(scp, p->right);
	int a = trace_ast_down_up_gen_3_address_code(scp, p->left);
	SymbolVar *symb = 0;
...
static void gen_scope_tac(Scope *scp)
{
...
//	}
	...
	for (auto it = scp->asts.begin(); it != scp->asts.end();)
	{
		...
		trace_ast_down_up_gen_3_address_code(scp, p);
		it++;
	}
